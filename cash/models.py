...
class CashChannel(models.Model):
    ...
    class Meta:
        verbose_name = "提现渠道类别设置"
        verbose_name_plural = "提现渠道类别设置"


class CashMoneyCategory(models.Model):
    gold = models.IntegerField(verbose_name="金额", default=0)
    first_created = models.DateTimeField(verbose_name="日期", auto_now_add=True)
    last_modify = models.DateTimeField(verbose_name="修改时间", auto_now=True)
    ...
    class Meta:
        verbose_name = "提现金额类别设置"
        verbose_name_plural = "提现金额类别设置"


class CashCategory(models.Model):
    channel = models.ForeignKey(CashChannel, verbose_name="提现方式")
    money_type = models.ManyToManyField(CashMoneyCategory, verbose_name="提现金额(分)")
